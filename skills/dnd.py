...
def run_skill(args_json):
    ...
    command = args_json.get('command', 'roll')
    commands = {
        'roll': cmd_roll,
        'spell': cmd_spell,
        'monster': cmd_monster,
    ...
    if command not in commands:
        print(json.dumps({"status": "error", "msg": f"unknown command: {command}"}))
        return
    ns = Namespace()
    if command == 'roll':
        ns.dice = args_json.get('dice', '1d20')
        ns.json = args_json.get('json', True)
    elif command == 'spell':
        ns.name = args_json.get('name')
        ns.search = args_json.get('search')
        ns.list = args_json.get('list', False)
        ns.json = args_json.get('json', True)
...
def main():
    # GenericAgent skill 协议: python dnd.py '{"command":"roll","dice":"2d6+3"}'

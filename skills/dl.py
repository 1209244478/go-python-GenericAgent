...
def run_skill(args_json):
    """GenericAgent skill 协议入口: 接收 JSON dict, 返回 JSON stdout."""
    from argparse import Namespace
    args = Namespace(
        url=args_json.get('url', ''),
        verbose=args_json.get('verbose', False),
        dry_run=args_json.get('dry_run', False),
        out_dir=args_json.get('out_dir', ''),
        music=args_json.get('music', False),
        video=args_json.get('video', False),
    )
    if not args.url:
        print(json.dumps({"status": "error", "msg": "url required"}))
        return
    logger.remove()
    ...
    try:
        ...
        if args.dry_run:
            print(json.dumps({"status": "success", "dry_run": True, "options": options}, ensure_ascii=False))
            return
        path = dl.download(options)
        print(json.dumps({"status": "success", "path": str(path)}, ensure_ascii=False))
    except Exception as e:
        print(json.dumps({"status": "error", "msg": str(e)}, ensure_ascii=False))


def main() -> None:
    # GenericAgent skill 协议: python dl.py '{"url":"https://...","music":true}'
    if len(sys.argv) >= 2 and sys.argv[1].startswith('{'):
        run_skill(json.loads(sys.argv[1]))
        return
    args = cli()
